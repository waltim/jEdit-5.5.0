...
public class PrintPreview extends EnhancedDialog
{
	...
	private void installListeners()
	{
		...
		pages.addKeyListener( (KeyEvent ke )->{ int selectedIndex = pages.getSelectedIndex();
		...
		prevPage.addActionListener( (ActionEvent ae )->{ int selectedIndex = pages.getSelectedIndex();
					if ( selectedIndex <= 0 )
					{
					...
					attributes.add( new PageRanges( selectedPage ) );
					printPreviewPane.setModel( model );});
		nextPage.addActionListener( (ActionEvent ae )->{ int selectedIndex = pages.getSelectedIndex();
					...
					attributes.add( new PageRanges( selectedPage ) );
					printPreviewPane.setModel( model );});
		zoomIn.addActionListener( (ActionEvent ae )->{ zoomLevel += 0.1f;
					int selectedPage = ( Integer )pages.getSelectedItem();
					model.setZoomLevel( zoomLevel );
					model.setPageNumber( selectedPage - 1 );
					model.setPageRanges( pageRanges );
					...
					attributes.add( new PageRanges( selectedPage ) );
					printPreviewPane.setModel( model );});
		zoomOut.addActionListener( (ActionEvent ae )->{ zoomLevel -= 0.1f;
					if ( zoomLevel <= 0.0f )
					{
						zoomLevel = 0.1f;
					}
					...
					attributes.add( new PageRanges( selectedPage ) );
					printPreviewPane.setModel( model );});
		fullWidth.addActionListener( (ActionEvent ae )->{ int selectedPage = ( Integer )pages.getSelectedItem();
					model.setPageNumber( selectedPage );
					model.setPageRanges( pageRanges );
					model.setZoom( PrintPreviewModel.Zoom.WIDTH );
					attributes.add( new PageRanges( selectedPage ) );
					printPreviewPane.setModel( model );});
		fullPage.addActionListener( (ActionEvent ae )->{ int selectedPage = ( Integer )pages.getSelectedItem();
					model.setPageNumber( selectedPage );
					model.setPageRanges( pageRanges );
