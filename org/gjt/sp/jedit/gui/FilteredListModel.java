	...
	public void setFilter(@Nullable final String filter)
	{
		Runnable runner = () -> {
                    ...
                    if (filter != null && !filter.isEmpty()) {
                        ...
                        Map<Integer, Integer> invertedIndices1 = new HashMap<Integer, Integer>();
                        for (int i = 0; i < size; i++) {
                            if (passFilter(i, prepped_filter)) {
                                Integer delegatedIndice = Integer.valueOf(i);
                                indices.add(delegatedIndice);
                                invertedIndices1.put(delegatedIndice, indices.size() - 1);
                            }
                        }
                        FilteredListModel.this.invertedIndices = invertedIndices1;
